...
import { childNoun, genderizeChildText } from '../domain/child';
...
export function generateReportHtml(data: ReportData): string {
  ...
  const labels = rtl ? {
    title: 'ملخص متابعة النمو', child: childNoun(profile.gender, lang), age: 'العمر', generated: 'تاريخ التقرير', stage: 'المرحلة المرجعية', blood: 'فصيلة الدم (حسب إدخال الأسرة، غير متحقق منها)',
    ...
    title: 'Development tracking summary', child: 'Child', age: 'Age', generated: 'Report date', stage: 'Reference stage', blood: 'Blood type (family-reported, unverified)',
  ...
  const statusLabel = (status?: MilestoneStatus) => childText(status === 'achieved' ? labels.achieved : status === 'emerging' ? labels.emerging : status === 'not_observed' ? labels.notObserved : labels.unrecorded);
  const rows = milestones.map((item) => `<tr><td>${esc(AGE_STAGES.find((entry) => entry.id === item.ageStageId)?.label[lang] ?? '')}</td><td>${esc(DOMAIN_LABELS[item.domain][lang])}</td><td>${esc(childText(item.title[lang]))}</td><td><span class="status ${milestoneStatuses[item.id] ?? 'unrecorded'}">${esc(statusLabel(milestoneStatuses[item.id]))}</span></td></tr>`).join('');
  const noteRows = observations.slice().reverse().map((item) => {
    ...
    return `<li><time>${esc(new Date(item.createdAt).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-GB'))}</time><p>${esc(item.text)}</p>${milestone ? `<small>${esc(childText(milestone.title[lang]))}</small>` : ''}</li>`;
  }).join('');
  ...
  @page{margin:28px}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;color:#3A2E2A;margin:0;line-height:1.55}header{border-bottom:4px solid #2F6F64;padding-bottom:18px;margin-bottom:22px}h1{margin:0;color:#7E352F;font-size:28px}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-top:14px}.meta div{background:#F7F3F0;padding:8px 12px;border-radius:8px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:9px;border-bottom:1px solid #E7DDD7;text-align:${rtl ? 'right' : 'left'}}th{background:#FBEAE4}.status{font-weight:700}.achieved{color:#2F6F50}.emerging{color:#855600}.not_observed{color:#A82E2A}.unrecorded{color:#6B5F5A}h2{font-size:17px;margin-top:24px}ul{padding-${rtl ? 'right' : 'left'}:20px}li{margin-bottom:12px}li p{margin:2px 0}time,small{color:#6B5F5A;font-size:11px}.notice{margin-top:24px;padding:12px;border:1px solid #855600;background:#FFF9E9;font-size:11px}.source{font-size:10px;color:#6B5F5A;margin-top:12px}</style></head><body>
  <header><h1>MUMAI · ${labels.title}</h1><div class="meta"><div><b>${labels.child}:</b> ${esc(profile.name)}</div><div><b>${labels.age}:</b> ${esc(formatAge(ageMonths, lang))}</div><div><b>${labels.stage}:</b> ${esc(stage.label[lang])}</div><div><b>${labels.generated}:</b> ${esc(date)}</div>${profile.bloodType ? `<div><b>${labels.blood}:</b> ${esc(profile.bloodType)}</div>` : ''}</div></header>
